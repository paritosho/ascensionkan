...
**********************************************************************************************************************************************/
	...
	var Opr5=(AgntUsr.indexOf('opera')!=-1||AgntUsr.indexOf('opera/5')!=-1)?1:0;	// Opera 5
...
function PositionMenuStruct(CntnrPntr,Tp,Lt){
	...
	while(Mmbr!=null){
		...
		if(Mmbr.ChildCntnr){
			if(Hori){
				SubTp=Tp+Topi+ChildVerticalOverlap*MmbrHeight+BorderWidth; 
//				SubTp=Tp+Topi+MmbrHeight+BorderWidth; 
				SubLt=Lt+Lefti+ChildOverlap*MmbrWidth
//				SubLt=Lt+Lefti
			}
			else{
				SubLt=Lt+Lefti+(1-ChildOverlap)*MmbrWidth+BorderWidth; SubTp=Tp+Topi+ChildVerticalOverlap*MmbrHeight
			}
...
function MemberSetUp(MmbrCntnr,PrMmbr,WhatMenu,Wdth,Hght){
	...
	if(MemVal.indexOf('<img')==-1&&eval(WhatMenu+'[2]')){
		S=(CreateRecursLevel==1&&FirstLineHorizontal)?relurl+'images/tridown.gif':relurl+'images/tri.gif';
...
function NavMemberSetUp(MmbrCntnr,PrMmbr,WhatMenu,Wdth,Hght){
	...
	if(this.value.indexOf('<img')==-1 &&eval(WhatMenu+'[2]')){
		...
		ImgStr=(CreateRecursLevel==1&&FirstLineHorizontal)?"<img src='" + relurl+ "images/tridown.gif'>":"<img src='" + relurl+ "" + relurl+ "tri.gif'>";
